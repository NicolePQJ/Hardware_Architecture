...
def parse_memory(stats_prefix):
    ...
    for layer in os.listdir(workload_dir_path):
        ...
        base_output_dir = []
        for hp_path in High_precision_dir.values():
            base_output_dir.append(os.path.join(hp_path, job_name))
...
#         baseline_DRAM_accesses = sum(j for j in baseline_dense_output_stats["energy_breakdown_pJ"]["DRAM"]["actual_accesses_per_instance"])
    ...
    #write memory values
    out_txt_path = os.path.join(LOW_PRECISION_OUT_DIR, "memory_output.txt")
    with open(out_txt_path, "a") as fh:
        fh.write(f"high_precison_8_bits_double_skipping_density_0.1\n")
        for job, energy_data in memory_values.items():
            #line = " ".join([job] + [str(v) for v in comp_map.values()])
            fh.write(f"{job}: {energy_data}\n")
...
def parse_energy(stats_prefix):
    stats_collector = {}
    low_precision_collector = {}
    ...
    for layer in os.listdir(workload_dir_path):
        ...
        for hp_path in High_precision_dir.values():
            base_output_dir.append(os.path.join(hp_path, job_name))
        low_precision_path = os.path.join(LOW_PRECISION_OUT_DIR, job_name)
...
#         workload_spec = yaml.load(open(os.path.join(workload_dir_path, layer)), Loader = yaml.SafeLoader)
        dense_iact = workload_spec["problem"]["instance"]["densities"]["Inputs"] > 0.9
        stats_collector[job_name] = {"path": base_output_dir, "dense_iact": dense_iact}
        low_precision_collector[job_name] = {"path": low_precision_path, "dense_iact": dense_iact}
    ...
    for job_name in job_names:
        job_info = stats_collector[job_name]
        energy = {}
        for i, density_path in enumerate(job_info["path"]):   
            job_output_stats = parse_timeloop_stats(os.path.join(density_path, "output", stats_prefix + ".map+stats.xml"))
            sparsity_ratio = list(High_precision_dir.keys())[i]
            energy[sparsity_ratio] = sum(energy["energy"] for energy in job_output_stats['energy_breakdown_pJ'].values())
            #print(f"{energy=}")
        energy_values[job_name] = energy

        lp_info = low_precision_collector[job_name]
        job_output_stats = parse_timeloop_stats(os.path.join(lp_info["path"], "output", stats_prefix + ".map+stats.xml"))
        lp_values[job_name] = sum(energy["energy"] for energy in job_output_stats['energy_breakdown_pJ'].values())
        #---------------------------------------------------------------------------')
    #write results to a text file
    # out_txt_path = os.path.join(LOW_PRECISION_OUT_DIR, "energy_output.txt")
    # with open(out_txt_path, "a") as fh:
    #     fh.write(f"high_precison_8_bits_double_skipping_density_ratio_0.1\n")
    ...
    energy_data_per_job = {}
    total_energy_usage = {}
    sparsities = list(energy_values[job_names[0]].keys())
    for job, energy_data in energy_values.items():
        lp_value = lp_values[job]
...
#     output_fig_2_path = os.path.join(job_info["path"], "output", "total_energy_plot_new_energy.png")
#     plt.savefig(output_fig_2_path, bbox_inches='tight')
#     plt.close()
#     print("Saved:", output_fig_2_path)
    fig, ax = plt.subplots(figsize=(12, 6))
    x          = np.arange(len(job_names))          # one tick per job
    n_comp     = len(sparsities)
    bar_width  = 0.8 / n_comp   

    for i, comp in enumerate(sparsities):
        vals = [energy_data_per_job[j][comp] for j in job_names]
        bars = ax.bar(x + i*bar_width, vals,
            width=bar_width,
            label=comp,#[f"{v:.0f}" for v in vals],
            color=plt.cm.tab10(i))
        ax.bar_label(                    # Matplotlib ≥3.4
            bars,
            labels=[f"{v/1e9:.2f}" for v in vals],   # format however you like
            padding=3,                           # distance above bar
            fontsize=8,
        )

    ax.set_xticks(x + bar_width*(n_comp-1)/2)
    ax.set_xticklabels(job_names)
    ax.set_ylabel("Total energy (pJ)")
    ax.set_xlabel("Job name")
    ax.set_title("Total energy usage per job for different sparsity ratios")
    ax.legend(title="Sparsity Ratio (High Precision / Low Precision)", bbox_to_anchor=(1.05, 1), loc="upper left")

    out2 = os.path.join(plot_dir, "total_energy_grouped.png")
    plt.savefig(out2, bbox_inches="tight")
    plt.close()
    print("Saved:", out2)

def main(stats_prefix):
    parse_energy(stats_prefix)
    #parse_memory(stats_prefix)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', type=str, default=os.path.join(this_directory, "..", "outputs", "low_precision"), help='abs path to top level output directory that needs to be parsed' )
    ...
    LOW_PRECISION_OUT_DIR = options.output_dir
    High_precision_dir = {
        0.8: os.path.join(this_directory, "..", "outputs", "density_0.8"),
        0.5: os.path.join(this_directory, "..", "outputs", "density_0.5"),
        0.3: os.path.join(this_directory, "..", "outputs", "density_0.3"),
        0.1: os.path.join(this_directory, "..", "outputs", "density_0.1"),
        0.01: os.path.join(this_directory, "..", "outputs", "density_0.01"),
    }
